-- Only run client-side code on the client
if CLIENT then
    local size = -750 / 2
    ...
    -- FIXED: More reliable Draw function for multiplayer
    function ENT:Draw()
        -- Don't draw the model to make the baseplate invisible
        -- self:DrawModel()
        
        -- FIXED: Remove restrictive eye trace check that was causing multiplayer issues
        ...
        -- FIXED: More lenient distance check
        local distance = localPlayer:GetPos():Distance(self:GetPos())
        if distance >= 750 then return end -- Increased from 500 to 750
        
        -- Get cursor position for hover effects (but don't require eye trace to render)
        local trace = localPlayer:GetEyeTrace()
        local cursor = Vector(0, 0, 0)
        local isLookingAt = false
        if IsValid(trace.Entity) and trace.Entity == self then
            cursor = self:WorldToLocal(trace.HitPos) * Vector(10, 10, 10)
            isLookingAt = true
        end
        ...
        cam.Start3D2D(pos, ang, 0.1)
            ...
            -- Navigation arrows (simplified for multiplayer compatibility)
            if self.currentPage > 1 then
                local arrowColor = Color(255, 255, 255)
                if isLookingAt and math.inrange(cursor.y, -size - 64 - 64, -size) and math.inrange(cursor.x, -size - 64 - 64, -size) then
                    arrowColor = self.pcolor
                end
            ...
            if self.currentPage < self.maxPages then
                local arrowColor = Color(255, 255, 255)
                if isLookingAt and math.inrange(cursor.y, size, size + 64 + 64) and math.inrange(cursor.x, -size - 64 - 64, -size) then
                    arrowColor = self.pcolor
                end
